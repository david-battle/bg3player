# Baldur's Gate 3 - Items Without Documented Acquisition

Two categories:

1. **Acquisition not documented** - these magic items exist but bg3.wiki does not record where/how they are obtained. Do not assume a location for them.

2. **Not obtainable in the current game** - Early Access-only items, inaccessible variants, or conjured items. Do not recommend these to a player.


## Not obtainable in the current game

### A Sparking Promise
- Rarity: Rare | Slot: Ring | Act: unknown
- Description: A Sparking Promise is a rare Ring. It grants a cantrip which can electrify surfaces the target walks on.
- Effect: Ensnaring Shock: Consume 4 of your Lightning Charges to mark a foe. When the target moves create an electrical blast with Range: 2 m (6.5 ft) radius that deals 1d4 Lightning. (Recharge: Short rest.)
- How to get: No documented acquisition; not found in the current game.

### Absolute's Smite
- Rarity: Uncommon | Slot: Ring | Act: unknown
- Description: Absolute's Smite is an inaccessible uncommon Ring that grants its wearer the ability to cast Searing Smite once per Long Rest.
- Effect: Searing Smite ( + + ): Cast as a level 1 spell (Recharge: Long rest.)
- How to get: Inaccessible item; not obtainable by the player.

### Allandra's Whelm
- Rarity: Rare | Slot: Weapon (Tridents) | Act: unknown
- Description: Allandra's Whelm is a rare, moderately enchanted (+2) variant of the Tridents family of weapons. It is a martial melee weapon that can be wielded in one hand, or with both hands for extra damage. Its design lends itself well to be thrown at enemies as a projectile.
- Effect: One-handed damage: 1d6 + 2 (3~8) + Strength modifier Piercing | Two-handed damage: 1d8 + 2 (3~10) + Strength modifier Piercing | Extra damage: 1d4 (1~4) Cold | Rush Attack: Charge forward and attack the first enemy in your way, possibly pushing them Off Balance. (Recharge: Short rest.) | Piercing Strike: Stab an enemy and possibly inflict Gaping Wounds. (Recharge: Short rest.) | Maiming Strike: Possibly Maims your target. They can't move. (Recharge: Short rest.) | Frigid Blade: You add your Proficiency Bonus as Cold Damage. When the attack hits, apply Frostbitten. The creature takes 1d4 Cold damage when making a weapon attack. (Recharge: Short rest.)
- How to get: No documented acquisition; not found in the current game.
- Bugs:
  - The spell applies Frostbitten, but the tooltip appears to invoke Frostbite.

### Arduous Flame Blade
- Rarity: Rare | Slot: Weapon (Longswords) | Act: unknown
- Description: Arduous Flame Blade is a rare, lightly enchanted (+1) variant of the Longswords family of weapons. It is a martial melee weapon that can be wielded in one hand, or with both hands for extra damage.
- Effect: One-handed damage: 1d8 (1~8) + Strength modifier Slashing | Two-handed damage: 1d10 (1~10) + Strength modifier Slashing | Pommel Strike: Make a non-lethal attack against an enemy and possibly Daze them. (Recharge: Short rest.) | Lacerate: Slash at your target's vital points to make it Bleed. (Recharge: Short rest.) | Rush Attack: Charge forward and attack the first enemy in your way, possibly pushing them Off Balance. (Recharge: Short rest.) | Flaming Blade: Immolate your blade like a phoenix wing and strike a foe with its blazing steel. (Recharge: Short rest.)
- How to get: No documented acquisition; not found in the current game.

### Battered Lute
- Rarity: Story Item | Slot: Instrument | Act: unknown
- Required Proficiency: Musical Instruments
- Description: Battered Lute is a uniquely named lute that is part of a quest. It is otherwise identical to the common item variation, and can be used as such.
- How to get:
  - On the corpse of Malus Thorm in the House of Healing
- Notes:
  - The Battered Lute is a necessary part of the Wake Art Cullagh objective of the Lift the Shadow Curse quest.
  - The party is awarded 35 experience when this item picked up.

### Blood-Bound Blade
- Rarity: Rare | Slot: Weapon (Longswords) | Act: unknown
- Description: Blood-Bound Blade is a rare, lightly enchanted (+1) special variant of the Longswords family of weapons. It is a martial melee weapon that can be wielded in one hand, or with both hands for extra damage.
- Effect: One-handed damage: 1d8 + 1 (2~9) + Strength modifier Slashing | Two-handed damage: 1d10 + 1 (2~11) + Strength modifier Slashing | Favoured Weapon: This weapon has a +1 bonus to damage and Attack Rolls. | Synchronicity: This weapon deals additional damage equal to your Proficiency Bonus. | Pommel Strike: Make a non-lethal attack against an enemy and possibly Daze them. (Recharge: Short rest.) | Lacerate: Slash at your target's vital points to make it Bleed. (Recharge: Short rest.) | Rush Attack: Charge forward and attack the first enemy in your way, possibly pushing them Off Balance. (Recharge: Short rest.) | This blade's magical powers only function if it is bound to an Eldritch Knight or is a Warlock's Pact Weapon.
- How to get: No documented acquisition; not found in the current game.
- Notes:
  - This weapon gains the condition Arcanely Honed if it's the target of Weapon Bond or Bind Pact Weapon.
- Bugs:
  - If this weapon is bound by an Eldritch Knight or Pact of the Blade Warlock, the weapon will retain its Favoured Weapon passive until the next long rest, as well as the bonus damage from Synchronicity, even if the character binds another weapon. The weapon will even retain this passive if the weapon is used by another character, different from the one who bound it.

### Briskwind Boots
- Rarity: Rare | Slot: Boots | Act: unknown
- Description: The Briskwind Boots is a rare pair of Boots that allows the wearer to regain all movement speed upon landing a critical hit or killing a hostile creature.
- Effect: Critical Swiftness: After scoring a Critical Hit or killing a hostile creature, movement speed is restored to maximum.
- How to get: No documented acquisition; not found in the current game.

### Chain of Liberation
- Rarity: Uncommon | Slot: Medium Armour (Medium) | Act: unknown
- Required Proficiency: Medium Armour
- Description: The Chain of Liberation is an uncommon Medium Armour that allows the wearer to sacrifice their Momentum (Condition) to rush forward and avoid Attacks of Opportunity.
- Effect: AC 13 | Sprint: Once per turn, the wearer can reduce the duration of their Momentum by 1 turn to Sprint.
- How to get:
  - Selûnite Outpost X: 175 Y: -248: in a chest within a hidden room (Early Access only)
- Notes:
  - Replaced by Luminous Armour in the full release of the game.
  - Does not provoke opportunity attacks.

### Circlet of Fire
- Rarity: Uncommon | Slot: Helmet | Act: unknown
- Description: The Circlet of Fire is an uncommon Circlet that grants the wearer a bonus action whenever they deal Fire damage with a leveled spell.
- Effect: Fiery Return: When you deal Fire damage with a levelled spell, you Burn yourself and gain an additional Bonus Action this round.
- How to get:
  - This item is not present in the launch version of the game. Its effects have largely been replicated by the Pyroquickness Hat found in Act Three.
  - In Early Access versions of the game the circlet could be found in a gilded chest in the final room of the Apothecary's Cellar beneath the Blighted Village X: -659 Y: -370. The cellar can be accessed through the building directly behind the village's Ancient Rune Circle.
- Notes:
  - The conditions for activating Fiery Return are to deal fire damage with a non-cantrip spell or a melee spell attack roll. Attacks with a Flame Blade conjured by the druid spell count as melee spell attacks, so they trigger Fiery Return. However, this is not true of a Flame Blade summoned by a mephistopheles tiefling's racial feature or a permanent Flame Blade acquired through the exploit described here.
  - The Daredevil Gloves can turn ranged spell attacks into melee spell attacks, so cantrips like Fire Bolt can trigger Fiery Return when paired with the gloves.
- Bugs:
  - If Fiery Return is triggered with a Bonus Action (for example, with Metamagic: Quickened Spell), the extra bonus action granted by this passive is wasted and consumed immediately.
  - Hellcrawler, a class action granted by the Helldusk Boots, triggers Fiery Return, essentially negating the bonus action cost.
  - Searing Smite does not trigger Fiery Return.

### Cloak of Avarice
- Rarity: Rare | Slot: Cloak | Act: unknown
- Description: The Cloak of Avarice is a rare cloak.
- How to get: No documented acquisition; not found in the current game.

### Combination Axe
- Rarity: Rare | Slot: Weapon (Battleaxes) | Act: unknown
- Description: Combination Axe is a rare, lightly enchanted (+1) variant of the Battleaxes family of weapons. It is a martial melee weapon that can be wielded in one hand, or with both hands for extra damage.
- Effect: One-handed damage: 1d8 + 1 (2~9) + Strength modifier Slashing | Two-handed damage: 1d10 + 1 (2~11) + Strength modifier Slashing | Critical Virtuoso: When you score a Critical Hit you can spend a Bonus Action to make an additional attack. | Cleave: Swing your weapon in a large arc to attack up to 3 enemies at once. They each take half the damage your weapon usually deals. (Recharge: Short rest.) | Lacerate: Slash at your target's vital points to make it Bleed. (Recharge: Short rest.) | Maiming Strike: Possibly Maims your target. They can't move. (Recharge: Short rest.) | Press the Advantage: After scoring a critical hit, use your Bonus Action to make an additional attack.
- How to get: No documented acquisition; not found in the current game.
- Notes:
  - This weapon action requires and consumes the Critical Hit! condition which is applied for 1 turn by landing a critical hit with the Combination Axe.

### Dauntless Amulet
- Rarity: Rare | Slot: Amulet | Act: unknown
- Description: Dauntless Amulet is a rare Amulet that prevents the wearer from being Frightened.
- Effect: Dauntless: You can't be Frightened or afflicted with other emotion-altering conditions. | Intelligence Saving Throws +1
- How to get: No documented acquisition; not found in the current game.
- Notes:
  - Dauntless prevents any condition in the Frightened or Fleeing status groups, as well as Hideous Laughter.

### Deadly Channeller Gloves
- Rarity: Rare | Slot: Gloves | Act: unknown
- Description: Deadly Channeller Gloves are a rare pair of Gloves that temporarily lower the user's critical hit threshold.
- Effect: Arcane Critical (+): Cast as a level 1 spell at will.
- How to get: No documented acquisition; not found in the current game.

### Doom Axe
- Rarity: Uncommon | Slot: Weapon (Greataxes) | Act: unknown
- Description: Doom Axe is an uncommon Greataxe that afflicts targets with Bone Chilled condition on each successful hit.
- Effect: Damage: 1d12 (1~12) + Strength modifier Slashing | Edge of Terror: Each strike fills the target with a chilling dread that prevents it from regaining hit points. Undead creatures also get Disadvantage on Attack Rolls. | Cleave: Swing your weapon in a large arc to attack up to 3 enemies at once. They each take half the damage your weapon usually deals. (Recharge: Short rest.) | Lacerate: Slash at your target's vital points to make it Bleed. (Recharge: Short rest.) | Prepare: Spend 6 m (20 ft) of your movement to deal an additional Strength modifier PhysicalDRS damage (minimum 1) on each successful melee weapon attack for the rest of the turn. (Recharge: Short rest.)
- How to get: Early Access only; replaced by Doom Hammer in the full release.
- Notes:
  - This item is not present in the launch version of the game. Its effects were largely replaced by the Doom Hammer, which is identical save for being a maul instead of an axe.

### Enforcer Club
- Rarity: Uncommon | Slot: Weapon (Clubs) | Act: unknown
- Description: Enforcer Club is an uncommon, lightly enchanted (+1) variant of the Clubs family of weapons. It is a simple melee weapon wielded in one hand. It is a light weapon that anyone can dual-wield without special training.
- Effect: Damage: 1d4 + 1 (2~5) + Strength modifier Bludgeoning | Non-Lethality: This weapon deals only Non-Lethal Damage. | Concussive Smash: Hit an enemy with all your might to deal damage and possibly Daze them. (Recharge: Short rest.) | Weakening Strike: Target an enemy's hands with a non-lethal attack and possibly inflict Weak Grip. (Recharge: Short rest.) | Backbreaker: Put extra force behind your strike to possibly knock your enemy Prone. (Recharge: Short rest.)
- How to get: Unobtainable; appears in Dammon's Act Two treasure table with quantity 0.
- Notes:
  - This item appears in Dammon's Act 2 treasure table, but with a quantity of 0

### Enforcer Helmet
- Rarity: Uncommon | Slot: Helmet | Act: unknown
- Required Proficiency: Medium Armour
- Description: Enforcer Helm is an uncommon Helmet that heals the wearer when knocking out creatures.
- Effect: Rejuvenating Takedown: Whenever the wearer knocks a creature unconscious, the wearer regains 1d6 Hit Points. | Constitution Saving Throws +1
- How to get: Unobtainable; appears in Dammon's Act Two treasure table with quantity 0.
- Notes:
  - This item appears in Dammon's Act Two treasure table, but with a quantity of 0

### Enforcer Shield
- Rarity: Uncommon | Slot: Shield | Act: unknown
- Required Proficiency: Shields
- Description: Enforcer Shield is an uncommon Shield.
- Effect: AC +2 | Staunch Blessing: When a creature misses the wielder with an attack, they gain a +1d4 bonus to their next Attack Roll that deals non-lethal damage. This bonus is relinquished when the wielder deals lethal damage.
- How to get: Unobtainable; appears in Dammon's Act Two treasure table with quantity 0.
- Notes:
  - This item appears in Dammon's Act 2 treasure table, but with a quantity of 0

### Executioner Sword
- Rarity: Rare | Slot: Weapon (Shortswords) | Act: unknown
- Description: Executioner Sword is a rare, lightly enchanted (+1) variant of the Shortswords family of weapons. It is a martial melee weapon wielded in one hand. As a finesse weapon, it can benefit from the wielder's Dexterity and not just their Strength. It is a light weapon that anyone can dual-wield without special training.
- Effect: Damage: 1d6 + 1 (2~7) + Strength or Dexterity modifier Piercing | Liberated Spellcasting: When you kill a creature, you can cast a Level 1 spell without expending a spell slot. | Flourish: Feint an attack to possibly throw your opponent Off Balance. (Recharge: Short rest.) | Piercing Strike: Stab an enemy and possibly inflict Gaping Wounds. (Recharge: Short rest.)
- How to get: No documented acquisition; not found in the current game.
- Notes:
  - Level 1 spells can be upcasted without expending a higher level spell slot, even if the caster has no spell slots remaining for that level.

### Featherlight Boots
- Rarity: Uncommon | Slot: Boots | Act: unknown
- Description: The Featherlight Boots are an uncommon pair of Boots that grants the wearer additional speed while greatly injured.
- Effect: Burst of Speed: When the wearer has 50% Hit Points or less, their Movement Speed is increased by 3 m (10 ft).
- How to get:
  - Rewarded by Sovereign Glut for completing Avenge Glut's Circle (Early Access only)
- Notes:
  - This reward was changed to Winter's Clutches in the full release of the game.
  - The quest reward was a choice between this item and Ring of Fire.

### Flail of Dread Skulls
- Rarity: Rare | Slot: Weapon (Flails) | Act: unknown
- Description: Flail of Dread Skulls is a rare, lightly enchanted (+1) variant of the Flails family of weapons. It is a martial melee weapon wielded in one hand.
- Effect: Damage: 1d8 + 1 (2~9) + Strength modifier Slashing | Tenacity: When you miss a melee attack, you deal Strength Modifier Bludgeoning damage (minimum of 1) anyway. | Concussive Smash: Hit an enemy with all your might to deal damage and possibly Daze them. (Recharge: Short rest.) | Weakening Strike: Target an enemy's hands with a non-lethal attack and possibly inflict Weak Grip. (Recharge: Short rest.)
- How to get: Cut content; appears to be an earlier, unfinished version of the Shattered Flail.
- Notes:
  - This weapon looks to be an earlier version of the Shattered Flail
  - This weapon also looks to be unfinished. The in-game quote is more like a description which is higly unusual.
  - The quote says it may deal Psychic damage or Paralyse. This is false it can only apply madness.
  - The Psychic damage is also not shown in the tooltip outside of the quote.
  - The calculation for the saving throw shown in the tooltip may be wrong, but always results in 13. It may for example say 8 + 3 + 3 = 13 and state modifiers for other abilities than wisdom.
  - If a weapon action misses, using Tenacity does not trigger its effects.
  - As Tenacity deals weapon damage even if an attack misses, it can trigger many on-hit effects from equipment. For example, from the Gloves of Power, the Callous Glow Ring, weapon coatings, or any infusion from the Ring of Elemental Infusion.
- Bugs:
  - As Horde Breaker (Melee) rolls invisible, zero-damage weapon attacks against all enemies in the Horde Breaker area of effect, Tenacity can be prompted to trigger against them even if the first attack hits. It is therefore possible to score 2 hits in one attack: the full attack against the original Horde Breaker target and a Tenacity hit against another target.

### Gargoyle Boots
- Rarity: Very Rare | Slot: Boots | Act: unknown
- Description: Gargoyle Boots are a very rare pair of Boots that lets the wearer cast Gargoyle's Countenance, granting them resistance to Bludgeoning, Slashing and Piercing damage.
- Effect: Gargoyle's Countenance: Cast as a level 4 spell (Recharge: Short rest.) | Dexterity Saving Throws +2
- How to get: No documented acquisition; not found in the current game.

### Githyanki Breastplate
- Rarity: Rare | Slot: Medium Armour (Medium) | Act: unknown
- Required Proficiency: Medium Armour
- Description: Githyanki Breastplate is a rare Medium Armour
- Effect: AC 14
- How to get: No documented acquisition; not found in the current game.

### Githyanki Longsword (Psionic)
- Rarity: Uncommon | Slot: Weapon (Longswords) | Act: unknown
- Description: Githyanki Longsword (Psionic) is an uncommon, lightly enchanted (+1) variant of the Longswords family of weapons. It is a martial melee weapon that can be wielded in one hand, or with both hands for extra damage.
- Effect: One-handed damage: 1d8 + 1 (2~9) + Strength modifier Slashing | Two-handed damage: 1d10 + 1 (2~11) + Strength modifier Slashing | Githborn Psionic Weapon: When wielded by a githyanki this weapon deals an additional 1d4 Psychic damage. | Pommel Strike: Make a non-lethal attack against an enemy and possibly Daze them. (Recharge: Short rest.) | Lacerate: Slash at your target's vital points to make it Bleed. (Recharge: Short rest.) | Rush Attack: Charge forward and attack the first enemy in your way, possibly pushing them Off Balance. (Recharge: Short rest.)
- How to get: No documented acquisition; not found in the current game.
- Notes:
  - The githyanki-exclusive bonuses from Githborn Psionic Weapon can also be enabled by being disguised as one. However, the wielder must be disguised before equipping the weapon for this to work.

### Gloves of Fire Resistance
- Rarity: Uncommon | Slot: Gloves | Act: unknown
- Description: The Gloves of Fire Resistance are an uncommon pair of gloves that grant the wearer Resistance to Fire damage while heavily wounded.
- Effect: Chilling Grace: When the wearer has 50% Hit Points or less, they have Resistance to Fire damage.
- How to get:
  - Rewarded by Derryth Bonecloak for completing Find the Mushroom Picker (Early Access only)
- Notes:
  - This reward was changed to Gloves of Uninhibited Kushigo in the full release of the game.
  - Quest reward was a choice between this item and Gloves of Flint and Steel

### Gloves of Flint and Steel
- Rarity: Uncommon | Slot: Gloves | Act: unknown
- Description: The Gloves of Flint and Steel are an uncommon pair of gloves that allow the wearer to set enemies ablaze whenever they deal Fire damage.
- Effect: Hot Little Hands: When the wearer deals Fire damage, the target has to succeed a Constitution Saving Throw or start Burning.
- How to get:
  - Rewarded by Derryth Bonecloak for completing Find the Mushroom Picker (Early Access only)
- Notes:
  - This reward was changed to Gloves of Uninhibited Kushigo in the full release of the game.
  - Quest reward was a choice between this item and Gloves of Fire Resistance.

### Gloves of Succour
- Rarity: Uncommon | Slot: Gloves | Act: unknown
- Required Proficiency: Medium Armour
- Description: The Gloves of Succour are an uncommon pair of gloves that grant a creature healed by a Paladin's Lay on Hands resistance against weapon attacks.
- Effect: Warding Hands: When you use your Channel Oath Charge spells, you gain Heroism. | Strength Saving Throws +1
- How to get: Early Access only; replaced by Gloves of Heroism.
- Notes:
  - This item was only obtainable in Early Access. It was replaced by Gloves of Heroism in the full release of the game.
  - "Channel Oath spells" includes all of the various Paladin Channel Oath abilities, which are generally not considered "spells" in D&D terminology. However, this does not include Oath Spells (the subclass-specific spells which are always prepared for a Paladin).

### Goblinbane Dagger
- Rarity: Uncommon | Slot: Weapon (Daggers) | Act: unknown
- Description: Goblinbane Dagger is an uncommon dagger that grants the wielder defensive capabilities against Goblins. Any goblin attacking the wielder of Goblinbane suffers from Disadvantage on its Attack Rolls.
- Effect: Damage: 1d4 (1~4) + Strength or Dexterity modifier Piercing | Goblin's Undoing: Goblins have Disadvantage on Attack Rolls against the wielder. | Piercing Strike: Stab an enemy and possibly inflict Gaping Wounds. (Recharge: Short rest.)
- How to get: No documented acquisition; not found in the current game.
- Notes:
  - This dagger is identical to Worgfang except for its name.

### Hag's Ring
- Rarity: Uncommon | Slot: Ring | Act: unknown
- Description: Hag's Ring is an uncommon Ring that grants the wearer access to Vicious Mockery
- Effect: Vicious Mockery: Cast as a cantrip at will.
- How to get: No documented acquisition; not found in the current game.

### Hat Of Uproarious Laughter
- Rarity: Rare | Slot: Helmet | Act: unknown
- Description: Hat Of Uproarious Laughter is a rare hat that tries to make targetted creatures fall on the floor laughting.
- Effect: Slippery Slapsticky: When knocked Prone, a hostile creature with an Intelligence of 5 or more within 9 m (30 ft) of user must succeed a Wisdom Saving Throw or become stricken with Laughter.
- How to get: No documented acquisition; not found in the current game.

### Headband of Intellect
- Rarity: Rare | Slot: Helmet | Act: unknown
- Description: The Headband of Intellect is a rare Circlet that raises the wearer's Intelligence to 19.
- Effect: Set the wearer's Intelligence score to 19. The enchantment has no effect if their Intelligence score is higher without it.
- How to get: Unobtainable version; the obtainable item is Warped Headband of Intellect.
- Notes:
  - This item was originally carried by Lump, before it was changed in Early Access to the Warped Headband of Intellect and its effect reduced to setting the Intelligence score to 17.

### Helm of Arcane Gate
- Rarity: Rare | Slot: Helmet | Act: unknown
- Description: The Helm of Arcane Gate is a rare Helmet that allows the wearer to cast Arcane Gate.
- Effect: Arcane Gate (+): Cast as a level 6 spell (Recharge: Long rest.)
- How to get: No documented acquisition; not found in the current game.

### Ilmater's Aid
- Rarity: Uncommon | Slot: Amulet | Act: unknown
- Description: Ilmater's Aid is an uncommon Amulet that allows the wearer to deal additional force damage with non-cantrip spells when their health drops below half.
- Effect: Strength in Suffering: Non-Cantrip spells deal an additional 1d4 Force damage when the wearer drops to half their hit points or lower.
- How to get:
  - In a gilded chest near Polma (Early Access only)
- Notes:
  - Replaced by Amulet of Misty Step after release.

### Infernal Longsword
- Rarity: Rare | Slot: Weapon (Longswords) | Act: unknown
- Description: Infernal Longsword is a rare, lightly enchanted (+1) variant of the Longswords family of weapons. It is a martial melee weapon that can be wielded in one hand, or with both hands for extra damage.
- Effect: One-handed damage: 1d8 + 1 (2~9) + Strength modifier Slashing | Two-handed damage: 1d10 + 1 (2~11) + Strength modifier Slashing | Infernal Weapon: On a hit, this weapon deals an additional 3 Poison damage and can possibly Poison its target. | Pommel Strike: Make a non-lethal attack against an enemy and possibly Daze them. (Recharge: Short rest.) | Lacerate: Slash at your target's vital points to make it Bleed. (Recharge: Short rest.) | Rush Attack: Charge forward and attack the first enemy in your way, possibly pushing them Off Balance. (Recharge: Short rest.)
- How to get: No documented acquisition; not found in the current game.
- Bugs:
  - The roll for the Poisoned saving throw is not shown in the combat log for either a successful or failed save. Because it has a low DC 12 Constitution Saving Throw, this can give the impression that the poison effect does not work at all.

### Infernal Warhammer
- Rarity: Rare | Slot: Weapon (Warhammers) | Act: unknown
- Description: Infernal Warhammer is a rare +1 Warhammer that deals an additional 3 Poison Damage on hit and has a chance to Poison targets it strikes.
- Effect: Damage: 1d8 + 1 (2~9) + Strength modifier Bludgeoning | Extra damage: 3 Poison | Infernal Weapon: On a hit, this weapon deals an additional 3 Poison damage and can possibly Poison its target. | Backbreaker: Put extra force behind your strike to possibly knock your enemy Prone. (Recharge: Short rest.) | Concussive Smash: Hit an enemy with all your might to deal damage and possibly Daze them. (Recharge: Short rest.) | Weakening Strike: Target an enemy's hands with a non-lethal attack and possibly inflict Weak Grip. (Recharge: Short rest.)
- How to get: No documented acquisition; not found in the current game.
- Bugs:
  - The roll for the Poisoned saving throw is not shown in the combat log for either a successful or failed save. Because it has a low DC 12 Constitution Saving Throw, this can give the impression that the poison effect does not work at all.

### Ironwood Breastplate
- Rarity: Uncommon | Slot: Medium Armour (Medium) | Act: unknown
- Required Proficiency: Medium Armour
- Description: Ironwood Breastplate is an uncommon Medium Armour that reduces Slashing damage by 1.
- Effect: AC 15 | Superior Material: You take 1 less Slashing damage. | Advantage on Constitution Saving Throws.
- How to get: No documented acquisition; not found in the current game.

### Kruznabir's Asylum Amulet
- Rarity: Rare | Slot: Amulet | Act: unknown
- Description: Kruznabir's Asylum Amulet is a rare Amulet that lets the user cause their targets to go mad.
- Effect: Crown of Madness (+): Cast as a level 2 spell (Recharge: Long rest.)
- How to get: No documented acquisition; not found in the current game.

### Light Crossbow of Speed
- Rarity: Rare | Slot: Weapon (Light_Crossbows) | Act: unknown
- Description: Light Crossbow of Speed is a rare Light Crossbow.
- Effect: Damage: 1d8 + 1 (2~9) + Dexterity modifier Piercing | Piercing Shot: Deal regular damage and possibly inflict Gaping Wounds which cause extra damage on attacks. (Recharge: Short rest.) | Blink-of-an-eye Bolt: Make a ranged attack with your equipped weapon.
- How to get: Legacy item from Baldur's Gate 1/2; no documented acquisition in BG3.
- Notes:
  - The Light Crossbow of Speed exists in Baldur's Gate and Baldur's Gate 2 - Shadows of Amn, also known as the The Army Scythe.

### Lofty Sorcerer's Amulet
- Rarity: Very Rare | Slot: Amulet | Act: unknown
- Description: Lofty Sorcerer's Amulet is a very rare inaccessible Amulet that grants the wearer an additional sorcery point.
- Effect: Lofty Sorcery: Grants an additional sorcery point to the wearer, which is restored on long rest along with your normal sorcery points.
- How to get: Inaccessible item; not obtainable by the player.

### Magical Hand Crossbow
- Rarity: Very Rare | Slot: Weapon (Hand_Crossbows) | Act: unknown
- Description: Magical Hand Crossbow is a very rare +2 Hand Crossbow that allows the wielder to shoot a target at close range and then make a melee attack against it as a reaction.
- Effect: Damage: 1d6 + 2 (3~8) + Dexterity modifier Piercing | Cunning Combo: When you shoot a target at close range, you can make an additional melee attack against it as a Reaction. | Piercing Shot: Deal regular damage and possibly inflict Gaping Wounds which cause extra damage on attacks. (Recharge: Short rest.) | Mobile Shot: After using Dash or Disengage this turn, you can make a ranged attack as a bonus action. (Recharge: Short rest.)
- How to get: No documented acquisition; not found in the current game.

### Mind Sundering Dagger
- Rarity: Uncommon | Slot: Weapon (Daggers) | Act: unknown
- Description: Mind Sundering Dagger is an uncommon, lightly enchanted (+1) variant of the Daggers family of weapons. It is a simple melee weapon wielded in one hand. As a finesse weapon, it can benefit from the wielder's Dexterity and not just their Strength. It is a light weapon that anyone can dual-wield without special training. Its design lends itself well to be thrown at enemies as a projectile.
- Effect: Damage: 1d4 + 1 (2~5) + Strength or Dexterity modifier Piercing | Extra damage: 1d4 (1~4) Psychic | Piercing Strike: Stab an enemy and possibly inflict Gaping Wounds. (Recharge: Short rest.)
- How to get: No documented acquisition; not found in the current game.

### Moonblade
- Rarity: Uncommon | Slot: Weapon (Longswords) | Act: unknown
- Description: Moonblade is an uncommon special variant of the Longswords family of weapons. It is a martial melee weapon that can be wielded in one hand, or with both hands for extra damage.
- Effect: One-handed damage: 1d8 (1~8) + Strength modifier Slashing | Two-handed damage: 1d10 (1~10) + Strength modifier Slashing | Lacerate: Slash at your target's vital points to make it Bleed. (Recharge: Short rest.) | Rush Attack: Charge forward and attack the first enemy in your way, possibly pushing them Off Balance. (Recharge: Short rest.) | Pommel Strike: Make a non-lethal attack against an enemy and possibly Daze them. (Recharge: Short rest.) | Melee Attack +1
- How to get: No documented acquisition; not found in the current game.

### Nightsinger's Half-Plate
- Rarity: Rare | Slot: Medium Armour (Medium) | Act: unknown
- Required Proficiency: Medium Armour
- Description: Nightsinger's Half-Plate is a rare Medium Armour that reduces Piercing damage by 2.
- Effect: AC 17 | Superior Plate: You take 2 less Piercing damage. | Disadvantage on Stealth checks.
- How to get: No documented acquisition; not found in the current game.

### Oathbreaker Knight Armour
- Rarity: Uncommon | Slot: Heavy Armour (Heavy) | Act: unknown
- Required Proficiency: Heavy Armour
- Description: Oathbreaker Knight Armour is a set of Heavy Armour, worn by the Oathbreaker Knight.
- Effect: AC 19 | Disadvantage on Stealth checks.
- How to get: Not lootable; the Oathbreaker Knight's armour is a visual override.
- Notes:
  - This armour is visually similar to the armor worn by Oathbreaker Knight, but the Oathbreaker's is a visual override on Plate Armour whereas this armor is based on Plate Armour +1. There are also slight visual differences (for example, in the belt).

### Planeslayer Flail
- Rarity: Rare | Slot: Weapon (Flails) | Act: unknown
- Description: Planeslayer Flail is a rare, moderately enchanted (+2) variant of the Flails family of weapons. It is a martial melee weapon wielded in one hand.
- Effect: Damage: 1d8 + 2 (3~10) + Strength modifier Bludgeoning | Outer-Entity Slayer: When wielded against fey, fiends, or undead foes, this weapon has a +1d4 bonus to Damage and Attack Rolls. | Tenacity: When you miss a melee attack, you deal Strength Modifier Bludgeoning damage (minimum of 1) anyway. | Concussive Smash: Hit an enemy with all your might to deal damage and possibly Daze them. (Recharge: Short rest.) | Weakening Strike: Target an enemy's hands with a non-lethal attack and possibly inflict Weak Grip. (Recharge: Short rest.)
- How to get: No documented acquisition; shares the Shattered Flail's Tenacity mechanic (unfinished/EA-era).
- Notes:
  - If a weapon action misses, using Tenacity does not trigger its effects.
  - As Tenacity deals weapon damage even if an attack misses, it can trigger many on-hit effects from equipment. For example, from the Gloves of Power, the Callous Glow Ring, weapon coatings, or any infusion from the Ring of Elemental Infusion.
- Bugs:
  - As Horde Breaker (Melee) rolls invisible, zero-damage weapon attacks against all enemies in the Horde Breaker area of effect, Tenacity can be prompted to trigger against them even if the first attack hits. It is therefore possible to score 2 hits in one attack: the full attack against the original Horde Breaker target and a Tenacity hit against another target.

### Promise
- Rarity: Uncommon | Slot: Weapon (Daggers) | Act: unknown
- Description: Promise is an uncommon, lightly enchanted (+1) variant of the Daggers family of weapons. It is a simple melee weapon wielded in one hand. As a finesse weapon, it can benefit from the wielder's Dexterity and not just their Strength. It is a light weapon that anyone can dual-wield without special training. Its design lends itself well to be thrown at enemies as a projectile.
- Effect: Damage: 1d4 + 1 (2~5) + Strength or Dexterity modifier Piercing | Shadow Blade: You have Advantage on Attack Rolls against Lightly or Heavily Obscured targets when using this blade. | Piercing Strike: Stab an enemy and possibly inflict Gaping Wounds. (Recharge: Short rest.)
- How to get: No documented acquisition; not found in the current game.
- Notes:
  - While active, Shadow Blade provides Advantage to both mainhand and off-hand equipped melee weapons.
  - Prior to Patch 8 Hotfix 1, Shadow Blade granted advantage to all attack rolls when active. This was changed to only melee weapon attacks.

### Rebound Battleaxe
- Rarity: Rare | Slot: Weapon (Battleaxes) | Act: unknown
- Description: Rebound Battleaxe is a rare, lightly enchanted (+1) special variant of the Battleaxes family of weapons. It is a martial melee weapon that can be wielded in one hand, or with both hands for extra damage.
- Effect: One-handed damage: 1d8 + 1 (2~9) + Strength modifier Slashing | Two-handed damage: 1d10 + 1 (2~11) + Strength modifier Slashing | Favoured Weapon: This weapon has a +1 bonus to damage and Attack Rolls. | It's All In The Wrists: This weapon has the Thrown property and deals an additional 1d4 Thunder damage when thrown. | Cleave: Swing your weapon in a large arc to attack up to 3 enemies at once. They each take half the damage your weapon usually deals. (Recharge: Short rest.) | Lacerate: Slash at your target's vital points to make it Bleed. (Recharge: Short rest.) | Maiming Strike: Possibly Maims your target. They can't move. (Recharge: Short rest.) | This blade's magical powers only function if it's Bound to an Eldritch Knight or is a Warlocks's Pact Weapon.
- How to get: No documented acquisition; not found in the current game.
- Bugs:
  - If this weapon is bound by an Eldritch Knight or Pact of the Blade Warlock, the weapon will retain its Favoured Weapon passive until the next long rest even if the character binds another weapon. The weapon will even retain this passive if the weapon is used by another character, different from the one who bound it.
  - If this weapon is bound by an Eldritch Knight or Pact of the Blade Warlock, this weapon will permanently retain the thunder damage bonus from It's All In The Wrists while the thrown property will remain until the next long rest even if the character binds another weapon. The weapon will even retain this passive if the weapon is used by another character, different from the one who bound it.

### Render of Scrumptious Flesh
- Rarity: Rare | Slot: Weapon (Longswords) | Act: unknown
- Description: Render of Scrumptious Flesh is a rare, moderately enchanted (+2) variant of the Longswords family of weapons. It is a martial melee weapon that can be wielded in one hand, or with both hands for extra damage.
- Effect: One-handed damage: 1d8 + 2 (3~10) + Strength modifier Slashing | Two-handed damage: 1d10 + 2 (3~12) + Strength modifier Slashing | Extra damage: 1d4 (1~4) Necrotic | Pommel Strike: Make a non-lethal attack against an enemy and possibly Daze them. (Recharge: Short rest.) | Lacerate: Slash at your target's vital points to make it Bleed. (Recharge: Short rest.) | Rush Attack: Charge forward and attack the first enemy in your way, possibly pushing them Off Balance. (Recharge: Short rest.) | Part the Flesh: Thrust forward with diabolical precision, possibly stymying your target's ability to heal. (Recharge: Short rest.)
- How to get: No documented acquisition; not found in the current game.

### Ring of Beguiling
- Rarity: Rare | Slot: Ring | Act: unknown
- Description: The Ring of Beguiling is an inaccessible rare Ring that allows the wearer to cast the Charm Person spell once per Long Rest.
- Effect: Charm Person (+): Cast as a level 1 spell (Recharge: Long rest.)
- How to get: Inaccessible item; not obtainable by the player.

### Ring of Crabsight
- Rarity: Uncommon | Slot: Ring | Act: unknown
- Description: Ring of Crabsight is an uncommon Ring that grants +2 Perception.
- Effect: Perception +2
- How to get: Cut content; intended for an unfinished/cut quest.
- Notes:
  - This ring's UID suggests it was intended for an unfinished/cut quest.

### Ring of Fire
- Rarity: Uncommon | Slot: Ring | Act: unknown
- Description: The Ring of Fire is an uncommon Ring that grants the wearer additional fire damage when dealing fire damage.
- Effect: Burning Fury: When dealing Fire damage, the wearer deals an additional 1 Fire.
- How to get:
  - Rewarded by Sovereign Glut for completing Avenge Glut's Circle (Early Access only)
- Notes:
  - This reward was changed to Winter's Clutches in the full release of the game.
  - The quest reward was a choice between this item and Featherlight Boots.

### Ring of Restorative Gravity
- Rarity: Uncommon | Slot: Ring | Act: unknown
- Description: The Ring of Restorative Gravity is an uncommon Ring that pulls creatures towards the wearer when healed by them.
- Effect: Saviour's Allure: You restore an additional 2 Hit Points every time you heal another creature.
- How to get: Early Access only.
- Notes:
  - This ring was only available in Early Access. It was sold by Omeluum in the Myconid Colony after completing his quest Help Omeluum investigate the parasite. In the Release Version, it was replaced by the Ring of Salving, though its 'gravity-themed' flavour text was retained.
  - Prior to release, Saviour's Allure had the following effect: When the wearer heals another creature, it is pulled 2m / 7ft towards them.

### Robe of Spell Resistance
- Rarity: Rare | Slot: Clothing | Act: unknown
- Description: Robe of Spell Resistance is a rare Clothing that allows the wearer to resist the effects of spells.
- Effect: AC 10 | Saving Throws +2
- How to get: No documented acquisition; not found in the current game.

### Robust Chain Shirt
- Rarity: Uncommon | Slot: Medium Armour (Medium) | Act: unknown
- Required Proficiency: Medium Armour
- Description: The Robust Chain Shirt is an uncommon Medium Armour that deals Piercing damage to enemies that damage the wearer with a melee attack while the wearer is greatly injured.
- Effect: AC 13 | Piercing Counter: When the wearer has 50% hit points or less and is damaged by a melee attack, they deal 1d4 Piercing to the attacker.
- How to get: Replaced; originally Sergeant Thrinn's reward, substituted with Armour of Uninhibited Kushigo.
- Notes:
  - Initially a reward given by Sergeant Thrinn for Find the Missing Boots, this armour was substituted for Armour of Uninhibited Kushigo.

### Sandthief's Ring
- Rarity: Uncommon | Slot: Ring | Act: unknown
- Description: The Sandthief's Ring is an inaccessible uncommon Ring that allows the wearer to cast the Invisibility spell once per Long Rest.
- Effect: Invisibility (+): Cast as a level 1 spell (Recharge: Long rest.)
- How to get: Inaccessible item; not obtainable by the player.

### Sanguine Blade
- Rarity: Uncommon | Slot: Weapon (Rapiers) | Act: unknown
- Description: Sanguine Blade is an uncommon Rapier that grants better Attack Rolls, damage, and Spell Save DC for every foe slain (up to 3).
- Effect: Damage: 1d8 + 1 (2~9) + Strength or Dexterity modifier Piercing | Scarlet Remittance: Gain a +1 bonus to Attack Rolls, damage, and Spell Save DC for every foe you slay, up to a maximum of +3. | Flourish: Feint an attack to possibly throw your opponent Off Balance. (Recharge: Short rest.) | Piercing Strike: Stab an enemy and possibly inflict Gaping Wounds. (Recharge: Short rest.) | Weakening Strike: Target an enemy's hands with a non-lethal attack and possibly inflict Weak Grip. (Recharge: Short rest.) | Scarlet Regeneration: If you have a +3 bonus from Scarlet Remittance, consume it to recover 3d10 Hit Points (Recharge: Long rest).
- How to get: No documented acquisition; not found in the current game.
- Notes:
  - As of Patch 5, Scarlet Remittance only works when killing living, hostile targets. Previously, this feature would trigger on anything with a healthbar, including objects like crates and pouches.
  - A creature wielding a weapon with Scarlet Remittance loses the bonus when the passive feature becomes unequipped.

### Shadow Battleaxe
- Rarity: Uncommon | Slot: Weapon (Battleaxes) | Act: unknown
- Description: Shadow Battleaxe is an uncommon battle axe.
- Effect: One-handed damage: 1d8 + 1 (2~9) + Strength modifier Slashing | Two-handed damage: 1d10 + 1 (2~11) + Strength modifier Slashing | Extra damage: 1d4 (1~4) Necrotic | Cleave: Swing your weapon in a large arc to attack up to 3 enemies at once. They each take half the damage your weapon usually deals. (Recharge: Short rest.) | Lacerate: Slash at your target's vital points to make it Bleed. (Recharge: Short rest.) | Maiming Strike: Possibly Maims your target. They can't move. (Recharge: Short rest.) | Bound Weapon
- How to get: No documented acquisition; not found in the current game.

### Shadow Blade (weapon)
- Rarity: Uncommon | Slot: Weapon (Shortswords) | Act: unknown
- Description: Shadow Blade is an uncommon Shortsword which grants the wielder advantage against Lightly or Heavily Obscured targets when using this blade. It also deals 2d8 Psychic damage instead of the usual 1d6.
- Effect: Damage: 2d8 (2~16) + Strength or Dexterity modifier Psychic | Shadow Blade: You have Advantage on Attack Rolls against Lightly or Heavily Obscured targets when using this blade. | Piercing Strike: Stab an enemy and possibly inflict Gaping Wounds. (Recharge: Short rest.) | Flourish: Feint an attack to possibly throw your opponent Off Balance. (Recharge: Short rest.)
- How to get:
  - Created via Shadow Blade spell
- Notes:
  - Unlike most Shortswords, Shadow Blade is classified as a Simple weapon and not a Martial weapon.
  - The summoner gains innate proficiency with this weapon so wizards and sorcerers who lack Shortsword proficiency can still use it to its fullest potential.
  - While this weapon can be dipped in most cases, it cannot be Dipped in Fire.
  - This weapon cannot be disarmed.
  - While active, Shadow Blade provides Advantage to both mainhand and off-hand equipped melee weapons.
  - Prior to Patch 8 Hotfix 1, Shadow Blade granted advantage to all attack rolls when active. This was changed to only melee weapon attacks.

### Shadowstep Boots
- Rarity: Uncommon | Slot: Boots | Act: unknown
- Description: Shadowstep Boots are an uncommon pair of Boots that allows the wearer to teleport to an obscured, unoccupied spot.
- Effect: Shadow Teleportation: Teleport to an unoccupied, obscured spot. (Recharge: Short rest.)
- How to get: No documented acquisition; not found in the current game.

### Singing Sword
- Rarity: Uncommon | Slot: Weapon (Longswords) | Act: unknown
- Description: Singing Sword is an uncommon, lightly enchanted (+1) special variant of the Longswords family of weapons. It is a martial melee weapon that can be wielded in one hand, or with both hands for extra damage.
- Effect: One-handed damage: 1d8 + 1 (2~9) + Strength modifier Slashing | Two-handed damage: 1d10 + 1 (2~11) + Strength modifier Slashing | Lacerate: Slash at your target's vital points to make it Bleed. (Recharge: Short rest.) | Rush Attack: Charge forward and attack the first enemy in your way, possibly pushing them Off Balance. (Recharge: Short rest.) | Pommel Strike: Make a non-lethal attack against an enemy and possibly Daze them. (Recharge: Short rest.) | Phalar Aluve: Sing | Phalar Aluve: Shriek
- How to get: Cut content; appears to be an earlier version of Phalar Aluve (grants neither Sing nor Shriek).
- Notes:
  - This sword is likely an earlier version of Phalar Aluve with a few notable differences: The sword lists Sing and Shriek as two separate class actions, but does not grant either when equipped.
  - The +1 Performance bonus is not present.
  - It does not count as a finesse weapon.
  - The description and UID imply a different backstory / history for the weapon.

### Spiderstep Staff
- Rarity: Uncommon | Slot: Weapon (Quarterstaves) | Act: unknown
- Description: Spiderstep Staff is an uncommon variant of the Quarterstaves family of weapons. It is a simple melee weapon that can be wielded in one hand, or with both hands for extra damage.
- Effect: One-handed damage: 1d6 (1~6) + Strength modifier Bludgeoning | Two-handed damage: 1d8 (1~8) + Strength modifier Bludgeoning | Spider Step: Immune to being Enwebbed. Your Movement Speed is not affected by Web surfaces. | Topple: Swipe at a creature to knock it Prone. (Recharge: Short rest.)
- How to get: Cut content; unused - intended for Minthara's trial.
- Notes:
  - Visual effects tied to this item appear in MOO_Execution suggesting it was intended to appear during Minthara's trial.
  - Blocks Difficult Terrain: Web instead of the usual Difficult Terrain condition.

### Steadfast Maul
- Rarity: Uncommon | Slot: Weapon (Mauls) | Act: unknown
- Description: Steadfast Maul is an uncommon, lightly enchanted Maul.
- Effect: Damage: 2d6 + 1 (3~13) + Strength modifier Bludgeoning | Tenacity: When you miss a melee attack, you deal Strength Modifier Bludgeoning damage (minimum of 1) anyway. | Backbreaker: Put extra force behind your strike to possibly knock your enemy Prone. (Recharge: Short rest.) | Concussive Smash: Hit an enemy with all your might to deal damage and possibly Daze them. (Recharge: Short rest.) | Steadfast Strike: Slam into a foe, preventing them (and yourself) from being moved involuntarily. (Recharge: Short rest.)
- How to get: No documented acquisition; shares the Shattered Flail's Tenacity mechanic (unfinished/EA-era).
- Notes:
  - If a weapon action misses, using Tenacity does not trigger its effects.
  - As Tenacity deals weapon damage even if an attack misses, it can trigger many on-hit effects from equipment. For example, from the Gloves of Power, the Callous Glow Ring, weapon coatings, or any infusion from the Ring of Elemental Infusion.
- Bugs:
  - As Horde Breaker (Melee) rolls invisible, zero-damage weapon attacks against all enemies in the Horde Breaker area of effect, Tenacity can be prompted to trigger against them even if the first attack hits. It is therefore possible to score 2 hits in one attack: the full attack against the original Horde Breaker target and a Tenacity hit against another target.

### Synaptic Needle Amulet
- Rarity: Rare | Slot: Amulet | Act: unknown
- Description: Synaptic Needle Amulet is a rare Amulet.
- Effect: Mind Tax: Cleave into a Charmed or Frightened foe's brain tissue and invigorate your body. (Recharge: Short rest.)
- How to get: No documented acquisition; not found in the current game.

### Tenacious Boots
- Rarity: Uncommon | Slot: Boots | Act: unknown
- Required Proficiency: Medium Armour
- Description: Tenacious Boots are an uncommon medium-armour footwear that grants the wearer Athletics +1 and a spell to plant the wearer's feet to the ground.
- Effect: Stand Like Stone: Spend 1 turn of Force Conduit to become a strong bulwark that cannot be moved unwillingly (Recharge: Short rest. | Athletics +1
- How to get: No documented acquisition; not found in the current game.

### The Clover
- Rarity: Legendary | Slot: Weapon (Scimitars) | Act: unknown
- Description: The Clover is a legendary, moderately enchanted (+2) special variant of the Scimitars family of weapons. It is a martial melee weapon wielded in one hand. As a finesse weapon, it can benefit from the wielder's Dexterity and not just their Strength. It is a light weapon that anyone can dual-wield without special training.
- Effect: Damage: 1d6 (1~6) + Strength or Dexterity modifier Slashing | True Strike: Cast as a cantrip (Recharge: Short rest.) | Organ Rearranger: Reduce the number you need to roll a Critical Hit while attacking by 1. This effect can stack. When you roll 2 damage or less, reroll the dice, taking the highest result. | Exploit Weakness: Creatures hit with this weapon receive Vulnerability to Piercing damage. | True Strike Riposte: When a creature misses you with a melee attack, you may retaliate and gain True Strike. | Flourish: Feint an attack to possibly throw your opponent Off Balance. (Recharge: Short rest.) | Lacerate: Slash at your target's vital points to make it Bleed. (Recharge: Short rest.) | Armour Class +1 (Off-Hand Only)
- How to get: No documented acquisition; appears to be an unfinished variant sharing the Knife of the Undermountain King's Organ Rearranger passive.
- Notes:
  - Organ Rearranger stacks with other passive effects that improve critical chance, including ranged attacks and spell attack rolls [3][4].
  - Organ Rearranger's damage reroll is only applied to melee weapon attacks. When dual-wielding melee weapons, both weapons will benefit. All damage dice attached to an attack can be rerolled, not just the weapon's base damage die.
  - The damage reroll is not normally applied to any other attack type, including throw attacks using the Knife of the Undermountain King itself. However, Organ Rearranger's damage reroll does stack with itself (via a second Knife of the Undermountain King due to the King's Knife or Invoke Duplicity bugs) or dual wielding with Twist of Fortune. The improvement of the second reroll is smaller than the first one.
  - Exploit Weakness is only applied by Bloodthirst and only when used in melee combat; throwing this weapon does not apply the passive.
  - The retalitory attack granted by True Strike Riposte expends a Reaction resource.

### The Fork-Lightning Fingers
- Rarity: Rare | Slot: Gloves | Act: unknown
- Description: The Fork-Lightning Fingers are a rare pair of gloves that give the wearer lightning damage to their unarmed attacks and the ability to cast Lightning Bolt as a level 3 spell once per Long Rest.
- Effect: Lightning Bolt (+): Cast as a level 3 spell (Recharge: Long rest.) | Lightning Strikes: Your unarmed attacks deal an additional 1d4 Lightning.
- How to get: No documented acquisition; not found in the current game.

### The Impaler
- Rarity: Very Rare | Slot: Weapon (Pikes) | Act: unknown
- Description: The Impaler is a very rare, inaccessible, moderately enchanted (+2) Pike that deals additional damage to targets at full hp and grants temporary hp on kills.
- Effect: Damage: 1d10 + 2 (3~12) + Strength modifier Piercing | Break the Unbroken: Deals an additional 1d10 Piercing to targets that still have all their hit points. | Death Drinker: Gain 2d10 temporary hit points when landing a killing blow on an enemy with this weapon. | Brace (Melee): Spend 6 m (20 ft) of your movement. For the rest of your turn, roll melee damage twice and use the highest result. (Recharge: Short rest.) | Piercing Strike: Stab an enemy and possibly inflict Gaping Wounds. (Recharge: Short rest.) | Rush Attack: Charge forward and attack the first enemy in your way, possibly pushing them Off Balance. (Recharge: Short rest.) | Bloodrender: Deal additional damage equal to your proficiency bonus. On a successful hit, you gain Bloodrender which lets you deal more damage at the cost of some hitpoints. (Recharge: Short rest.)
- How to get: Inaccessible item; not obtainable by the player.
- Notes:
  - The target must be an enemy and killed by the damage dealt by the weapon for the effect to work.

### Tightening Orbit Helm
- Rarity: Rare | Slot: Helmet | Act: unknown
- Required Proficiency: Medium Armour
- Description: Tightening Orbit Helm is a rare Helmet.
- Effect: Tightening Orbit: Spend 1 turn of Force Conduit to pull every enemy in the radius toward you with damaging force. (Recharge: Short rest.) | Constitution Saving Throws +1
- How to get: No documented acquisition; not found in the current game.

### Torment Drinker Armour
- Rarity: Rare | Slot: Light Armour (Light) | Act: unknown
- Required Proficiency: Light Armour
- Description: Torment Drinker Armour is a rare Light Armour that grants temporary hit points when the wearer scores a Critical Hit or kills a creature.
- Effect: AC 13 | Bolstering Critical: When scoring a Critical Hit or killing a hostile creature, wearer gains 15 temporary hit points.
- How to get: No documented acquisition; not found in the current game.

### Tough Sunrises
- Rarity: Uncommon | Slot: Weapon (Morningstars) | Act: unknown
- Description: Tough Sunrises is an uncommon, lightly enchanted (+1) variant of the Morningstars family of weapons. It is a martial melee weapon wielded in one hand.
- Effect: Damage: 1d8 + 1 (2~9) + Strength modifier Piercing | Tenacity: When you miss a melee attack, you deal Strength Modifier Bludgeoning damage (minimum of 1) anyway. | Heartstopper: Smash an enemy's chest in and possibly inflict Chest Trauma. (Recharge: Short rest.) | Concussive Smash: Hit an enemy with all your might to deal damage and possibly Daze them. (Recharge: Short rest.) | Dawnburst Strike: Deal additional Radiant damage equal to your proficiency bonus. On a hit, light erupts around you in a 3 m (10 ft) area. Enemies in the light must succeed a Constitution Saving Throw or be Blinded. (Recharge: Short rest.)
- How to get: No documented acquisition; shares the Shattered Flail's Tenacity mechanic (unfinished/EA-era).
- Notes:
  - If a weapon action misses, using Tenacity does not trigger its effects.
  - As Tenacity deals weapon damage even if an attack misses, it can trigger many on-hit effects from equipment. For example, from the Gloves of Power, the Callous Glow Ring, weapon coatings, or any infusion from the Ring of Elemental Infusion.
- Bugs:
  - As Horde Breaker (Melee) rolls invisible, zero-damage weapon attacks against all enemies in the Horde Breaker area of effect, Tenacity can be prompted to trigger against them even if the first attack hits. It is therefore possible to score 2 hits in one attack: the full attack against the original Horde Breaker target and a Tenacity hit against another target.
  - The description in-game notes an incorrect DC 13 Constitution Saving Throw save. It requires a weapon action save with a +2 DC bonus.
  - Despite the in-game description stating only enemies in the light must save against becoming blinded, the blindness affects all creatures in the area, even allies. Due to a coding error, blindness does not affect the original target of the attack.
  - The additional Radiant damage is treated as a separate damage rider source even in Honour Mode.

### Treacleflow Amulet
- Rarity: Rare | Slot: Amulet | Act: unknown
- Description: Treacleflow Amulet is a rare Amulet.
- Effect: Rampart Aura: Costs Force Conduit to remain active. All entities with or in this aura have their movement speed halved.
- How to get: No documented acquisition; not found in the current game.

### Vampiric Gloves
- Rarity: Very Rare | Slot: Gloves | Act: unknown
- Description: Vampiric Gloves are a pair of Very Rare Gloves that grant the wearer the ability to cast a Vampiric Touch.
- Effect: Vampiric Touch (+): Cast as a level 3 spell (Recharge: Long rest.)
- How to get: Cut content; intended for an unfinished Cazador's Palace area.
- Notes:
  - This item's presence in LOW_Cazadors Palace_Crypt_Treasure suggests it was intended for Cazador's Palace, potentially the unfinished subterranean puzzle area

### Verminsign
- Rarity: Uncommon | Slot: Weapon (Rapiers) | Act: unknown
- Description: Verminsign is an uncommon, lightly enchanted (+1) variant of the Rapiers family of weapons. It is a martial melee weapon wielded in one hand. As a finesse weapon, it can benefit from the wielder's Dexterity and not just their Strength.
- Effect: Damage: 1d8 + 1 (2~9) + Strength or Dexterity modifier Piercing | Piercing Strike: Stab an enemy and possibly inflict Gaping Wounds. (Recharge: Short rest.) | Weakening Strike: Target an enemy's hands with a non-lethal attack and possibly inflict Weak Grip. (Recharge: Short rest.) | Flourish: Feint an attack to possibly throw your opponent Off Balance. (Recharge: Short rest.)
- How to get: No documented acquisition; not found in the current game.
- Notes:
  - Mechanically identical to a Rapier +1[verify]

### Vicious Shortsword
- Rarity: Very Rare | Slot: Weapon (Shortswords) | Act: unknown
- Description: Vicious Shortsword is a very rare, moderately enchanted (+2) variant of the Shortswords family of weapons. It is a martial melee weapon wielded in one hand. As a finesse weapon, it can benefit from the wielder's Dexterity and not just their Strength. It is a light weapon that anyone can dual-wield without special training.
- Effect: Damage: 1d6 + 10 (11~16) + Strength or Dexterity modifier Slashing | Dolor Amarus: When you land a Critical Hit with a weapon or unarmed attack, it deals an additional 7 damage. | Flourish: Feint an attack to possibly throw your opponent Off Balance. (Recharge: Short rest.) | Piercing Strike: Stab an enemy and possibly inflict Gaping Wounds. (Recharge: Short rest.)
- How to get: No documented acquisition; not found in the current game.
- Notes:
  - Dolor Amarus applies to all weapon attacks while a weapon with the feature is equipped, and it can be activated multiple times if multiple weapons with the passive are equipped. For example, if both Dolor Amarus daggers are equipped – or one dagger and the Vicious Battleaxe on a character with Dual Wielder – together with the Vicious Shortbow, when the wielder lands a critical hit, then +21 damage will be added, instead of +7.
  - Dolor Amarus also stacks on multiple damage riders, which could potentially lead to massive bonuses from different damage sources.
  - The name dolor amarus is Latin for "harsh suffering" or "bitter sorrow".
- Bugs:
  - Despite the description, Dolor Amarus does not deal an additional 7 damage on unarmed attack critical hits (including Throw attacks).

### Wakeful Amulet
- Rarity: Uncommon | Slot: Amulet | Act: unknown
- Description: Wakeful Amulet is an uncommon Amulet that prevents the user from being put to sleep.
- Effect: Wakefulness: You can't be unwillingly put to Sleep.
- How to get: No documented acquisition; not found in the current game.
